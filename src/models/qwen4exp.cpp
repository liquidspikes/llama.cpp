...
void llama_model_qwen4exp::load_arch_tensors(llama_model_loader & ml) {
    ...
    auto load_block = [&](int il, int flags, bool mtp) {
        ...
        // NextN/MTP blocks are always dense attention (is_recr is false for il >= n_layer)
        if (mtp || !hparams.is_recr(il)) {
            ...
            layer.index_q_proj = create_tensor(tn(LLM_TENSOR_INDEXER_Q_PROJ, "weight", il), { n_embd, hparams.indexer_n_head * idx_dim }, flags | TENSOR_NOT_REQUIRED);
            layer.index_k_proj = create_tensor(tn(LLM_TENSOR_INDEXER_K_PROJ, "weight", il), { n_embd, idx_dim }, flags | TENSOR_NOT_REQUIRED);
            layer.index_q_norm = create_tensor(tn(LLM_TENSOR_INDEXER_Q_NORM, "weight", il), { idx_dim }, flags | TENSOR_NOT_REQUIRED);
            layer.index_k_norm = create_tensor(tn(LLM_TENSOR_INDEXER_K_NORM, "weight", il), { idx_dim }, flags | TENSOR_NOT_REQUIRED);
        } else {
        ...
        if (mtp) {
            // hnorm is hc_dim (wide residual); enorm is n_embd (token embed)
    ...
    for (int i = 0; i < n_layer; ++i) {
        load_block(i, trunk_flags, false);
    }
    for (int i = n_layer; i < n_layer_all; ++i) {
        load_block(i, mtp_flags, true);
    }
}

std::unique_ptr<llm_graph_context> llama_model_qwen4exp::build_arch_graph(const llm_graph_params & params) const {
    if (params.gtype != LLM_GRAPH_TYPE_DECODER_MTP && hc_head_norm == nullptr) {
...
llama_model_qwen4exp::graph::graph(const llama_model & model, const llm_graph_params & params) :
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        // keep all rows when the MTP seed needs one h_nextn vector per batch token
        const bool mtp_keep_all = cparams.embeddings_nextn && !cparams.embeddings_nextn_masked;
        if (il == n_layer - 1 && inp_out_ids && !mtp_keep_all) {
    ...
    // MTP draft consumes the wide HC residual (n_embd_out = hc * n_embd), not the mixed stream
    {
        ggml_tensor * h_nextn = ggml_reshape_2d(ctx0, ggml_cont(ctx0, res_hc), n_embd * hc, res_hc->ne[2]);
...
void llama_model_qwen4exp::graph::build_mtp() {
    ...
    if (ubatch.token) {
        ...
        tok_embd = ggml_view_2d(ctx0, inp->embd, n_embd, n_tokens, ggml_row_size(inp->embd->type, hc_dim), 0);
    }
    ...
    // h is the wide residual [hc_dim, T]; grouped RMS + mean-pool to n_embd for eh_proj
    ggml_tensor * h_3d = ggml_reshape_3d(ctx0, h_in, n_embd, hc, n_tokens);
    ggml_tensor * h_n  = ggml_rms_norm(ctx0, h_3d, hparams.f_norm_rms_eps);
    h_n = ggml_reshape_2d(ctx0, h_n, hc_dim, n_tokens);
    h_n = ggml_mul(ctx0, h_n, layer.nextn.hnorm);
    h_n = ggml_reshape_3d(ctx0, h_n, n_embd, hc, n_tokens);
    ggml_tensor * mixed_h = ggml_view_2d(ctx0, h_n, n_embd, n_tokens,
            ggml_row_size(h_n->type, n_embd) * hc, 0);
    for (int64_t c = 1; c < hc; ++c) {
        ggml_tensor * v = ggml_view_2d(ctx0, h_n, n_embd, n_tokens,
                ggml_row_size(h_n->type, n_embd) * hc,
                ggml_row_size(h_n->type, n_embd) * c);
        mixed_h = ggml_add(ctx0, mixed_h, v);
    }
    mixed_h = ggml_scale(ctx0, mixed_h, 1.0f / (float) hc);
    cb(mixed_h, "mtp_hnorm", il);

    ggml_tensor * e_norm = build_norm(tok_embd, layer.nextn.enorm, nullptr, LLM_NORM_RMS, il);
    cb(e_norm, "mtp_enorm", il);

    ggml_tensor * concat = ggml_concat(ctx0, e_norm, mixed_h, /*dim=*/ 0);
    ...
    if (head_norm == nullptr) {
        const llama_model & other = qwen4exp_shared_model(cparams, model, "hc_head_norm.weight");
        head_norm = other.hc_head_norm;
        head_down = other.hc_head_down;
        head_up   = other.hc_head_up;
    }
...
ggml_tensor * llama_model_qwen4exp::graph::build_attn_qsa(
    ...
    if (top_k) {
        ...
        mask = ggml_add(ctx0, kq_mask_top_k, kq_mask);
    }
    ...
    ggml_tensor * cur = build_attn_mha(q, k, v, nullptr, mask, nullptr, nullptr, 0, kq_scale, il);
    cb(cur, "kqv_out", il);
